...
class User_type_model extends CI_Model {
    ...
    private $table_name = 'que_user_types';
    ...
    function delete_user_type($id, $data) {
        $this->db->where('id', $id);
        if ($this->db->update($this->table_name, $data)) {
            return 1;
        }
    ...
    function get_all_user_types($client_id) {
        $this->db->select($this->table_name . '.*');
        $this->db->where($this->table_name . '.is_deleted', 0);
    ...
    function get_all_active_user_types($client_id) {
        $this->db->select($this->table_name . '.*');
        $this->db->where($this->table_name . '.is_deleted', 0);
    ...
    function is_user_type_available($user_type_name, $client_id) {
